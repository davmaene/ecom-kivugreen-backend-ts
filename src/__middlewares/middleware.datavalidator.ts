...
export const validateCategoryCoopec = async (v: string) => {
    const categs = (await Services.rawCategoriesCoopecAsTableOfIds())
    v = (v ? v : '0')
...
export const coopecModelValidator = [
    ...
    body('id_category').notEmpty().isNumeric().isLength({ max: 100, min: 1 }).custom(validateCategoryCoopec).withMessage("`id_category` the value for id_category is not invalid ! this can only be a number min 1 max 100"),
];
